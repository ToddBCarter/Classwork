/*********************************************
 * Id: cart7982
 *
 * Compile: gcc -Wall
 * Run: ./a.out input.txt
 *
 * This is a basic tokenizer program that uses
 * a simple lexer and tokenizer function.
 *********************************************/

#define MAXTOKEN 256

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>

void lex(char*, int, int, int);
void tokenize(char*, int);

//This is main.  It dictates and handles the file to be used
//for data by the tokenizer.
int main(int argc, char *argv[])
{
   if(argc<2)
   {
      printf("Please specify input file.\n");
      printf("%s /y/shared/Engineering/cs-drbc/assignments/cs210/x01_in1.txt\n", argv[0]);
      return 1;
   }
   FILE * fp;
   char * line = NULL;
   size_t len = 0;
   ssize_t read;

   fp = fopen(argv[1], "r");
   if (fp == NULL){
      printf("Error: Could not open file %s\n", argv[1]);
      exit(EXIT_FAILURE);
   }

   while ((read = getline(&line, &len, fp)) != -1) {
      tokenize(line, read);
   }

   fclose(fp);
   if (line) {
      free(line);
   }
   exit(EXIT_SUCCESS);
}


//This is a basic lexer.  It removes whitespaces and
//uses strncpy to fill out the char array token.
void lex (char *line, int start, int end, int length)
{
   char token[MAXTOKEN];
   
   while(start < length && isspace(line[start]))
   {      
      start++;
   }
   
   if (start > end)
   {
      return;
   }

   printf("start is: %i", start);
   printf("end is: %i", end);

   strncpy(token, &line[start], end - start);

   token[end-start] = '\0';  
   
   printf ("Token: %s\n", token);
}


//This is the tokenizer.  It's only purpose is
//to run the lexer for the length of the line.
void tokenize(char *line, int length)
{
   int start = 0;
   int end =0;
   for(end = 0; end < length; end++)
   {
      if (isspace(line[end]))
      {
         lex(line, start, end, length);
         start = end;
      }
   }
}
